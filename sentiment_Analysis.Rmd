...
#cleaning 1: remove 'https:/....'
polit_tweets <- polit_tweets_ %>% 
...
#remove emoticons
# Clean text to remove odd characters (not working!)

polit_tweets$text <- sapply(polit_tweets$text,function(row) iconv(row, "latin1", "ASCII", sub=""))
...
```{r}
# Remove retweets 
polit_tweets_organic <- polit_tweets[polit_tweets$isRetweet==FALSE, ] 
...
bing_word_counts <- polit_tweets_organic_clean %>%
  ...
  count(word, sentiment, day, party_area, sort = TRUE) %>%
  group_by(sentiment) %>%
  ungroup() %>%
  mutate(word = reorder(word, n)) %>%
  ...
  mutate(senti_date_party = paste0(sentiment,"-", day, "-", party_area)) %>%
...
```{r}
bing_word_counts$senti_date_party <- factor(bing_word_counts$senti_date_party,
       levels = unique(bing_word_counts$senti_date_party))

# group by month and sentiment and then plot top 5 words each month
bing_word_counts %>%
  ggplot(aes(word, n, fill = sentiment)) +
  geom_col(show.legend = FALSE) +
  facet_wrap(~senti_date_party, scales = "free_y", ncol = 2) +
  labs(title = "Positive and negative sentiment for each political party",
